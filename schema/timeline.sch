...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sqf="http://www.schematron-quickfix.com/validator/process" queryBinding="xslt3">
    ...
    <pattern id="div-head-checks">
        ...
        <rule context="tei:div[not(@type)]">
            <assert test="tei:head/tei:date/(@when or (@from and @to) or (@notBefore and @notAfter))">Missing required attributes in entry head/date: a @when, or a @from and @to, or a
    ...
    <pattern id="date-alignment-checks">
        <rule context="tei:date[@when]">
            <assert test="normalize-space(.) = format-date(@when cast as xs:date, '[MNn] [D], [Y]')">Expected exact date @when=<value-of select="@when"/> to be formatted as “<value-of select="format-date(@when cast as xs:date, '[MNn] [D], [Y]')"/>”. Actual value: “<value-of select="."/>”.</assert>
        </rule>
        ...
        <rule context="tei:date[@notBefore and @notAfter]">
            ...
            <let name="expected-regex" value="                     (: Year :)                     if ($notBefore?year eq $notAfter?year and $notBefore?month eq 1 and $notAfter?month eq 12 and $notBefore?day eq 1 and $notAfter?day eq 31) then                         format-date($notBefore?date, '[Y]')                     else (: Month Year :)                         if ($notBefore?year eq $notAfter?year and $notBefore?month eq $notAfter?month and $notBefore?day eq 1 and $notAfter?day eq $notAfter?days-in-month) then                             format-date($notBefore?date, '[MNn] [Y]')                         else (: Month1–Month2, Year :)                             if ($notBefore?year eq $notAfter?year and $notBefore?month ne $notAfter?month and $notBefore?day eq 1 and $notAfter?day eq $notAfter?days-in-month) then                                 format-date($notBefore?date, '[MNn]') || '–' || format-date($notAfter?date, '[MNn], [Y]')                             else (: Year1–Year2 :)                                 if ($notBefore?year ne $notAfter?year and $notBefore?month eq 1 and $notAfter?month eq 12 and $notBefore?day eq 1 and $notAfter?day eq 31) then                                     format-date($notBefore?date, '[Y]') || '–' || format-date($notAfter?date, '[Y]')                                 else (: Month1 Year1–Month2 Year2 :)                                     if ($notBefore?year ne $notAfter?year and $notBefore?month ne $notAfter?month and $notBefore?day eq 1 and $notAfter?day eq $notAfter?days-in-month) then                                         format-date($notBefore?date, '[MNn] [Y]') || '–' || format-date($notAfter?date, '[MNn] [Y]')                                     else                                         'Sorry! Unrecognized uncertain date range pattern. Let Joe know!'"/>
            <assert role="warning" test="matches(normalize-space(.), '^' || $expected-regex || '$')">Expected uncertain date range @notBefore=<value-of select="@notBefore"/> @notAfter=<value-of select="@notAfter"/> to be formatted as “<value-of select="$expected-regex"/>”. Actual value: “<value-of select="."/>”.</assert>
        </rule>
    </pattern>
